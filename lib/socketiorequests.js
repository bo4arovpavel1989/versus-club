...
Newsschema.find({}, function(err, rep){
	console.log('news are:/n ' + rep);
});
	Userschema.findOne({loginUpperCase: '@MASTER'}, 'isModerator login', function(err, rep){
						if (rep !== null) {
							if (rep.isModerator == false) {
								Userschema.update({loginUpperCase: '@MASTER'}, {$set: {isModerator: true}}, function(err, rep2){
...
var transporter = nodemailer.createTransport(smtpTransport(smtpConfig));
 ...
 var socketioRequests  = function (client) {
	...
	client.on('deleteComment', function(data, data2){
		try {
			console.log('deleteData:/n' + data._id + ' ' + data.commentCount);
