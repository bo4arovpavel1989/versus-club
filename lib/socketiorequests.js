...
var Clientschema = require('./models/clientMongoSchema.js');
...
var transporter = nodemailer.createTransport(smtpTransport(smtpConfig));
 ...
 var socketioRequests  = function (client, scServer) {
	...
	client.on('getData', function(data){
		try {
			...
			Userschema.findOne({session: data}, function(err, rep){ /*ищем нужную сессию - к ней привязан ключ юзера*/
				console.log(rep);
				if (rep != null) {
					Clientschema.update({loginUpperCase:rep.loginUpperCase},{$set:{socket_id:client.id}},{upsert:true}).exec();
		...
		try {
			...
			Userschema.findOne({loginUpperCase: loginToEnter}, 'password', function(err, reply){
				if (reply !== null) {
					...
					Clientschema.update({loginUpperCase:loginToEnter},{$set:{socket_id:client.id}},{upsert:true}).exec();
					...
					Userschema.findOne({loginUpperCase: banNick}, 'isBanned login', function(err, rep){
						if (rep !== null) {
							if (rep.isBanned == false) {
								...
								Clientschema.findOne({loginUpperCase:banNick},function(err,rep2){
									if(rep2){
										console.log(rep2);
										scServer.clients[rep2.socket_id].emit('banRealTime');
									}
								});
							} else {
								...
								Clientschema.findOne({loginUpperCase:banNick},function(err,rep2){
									if(rep2){
										console.log(rep2);
										scServer.clients[rep2.socket_id].emit('banCancelRealTime');
									}
								});
							}
						}
	...
	client.on('disconnect',function(){
		Clientschema.remove({socket_id:client.id}).exec();
	});
