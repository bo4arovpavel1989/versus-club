					...
					Userschema.findOne({loginUpperCase: banNick}, 'isBanned login', function(err, rep){
						if (rep !== null) {
							if (rep.isBanned == false) {
								...
								Clientschema.find({loginUpperCase:banNick},function(err,rep2){
									if(rep[2]){
										scServer.clients[rep[2].socket_id].emit('banRealTime');
									}
								});
							} else {
								...
								Clientschema.find({loginUpperCase:banNick},function(err,rep2){
									if(rep[2]){
										scServer.clients[rep[2].socket_id].emit('banCancelRealTime');
									}
								});
							}
						}
