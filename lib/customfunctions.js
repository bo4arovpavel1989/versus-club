...
var simpleVerificate = function (data, callback){
	var reply;
	Userschema.findOne({_id: data._id}, 'session isBanned', function (err, person) {
		console.log(person);
		if (person !== null) {
			try {
				if (person.session == data.session && person.session != 0 && !person.isBanned) {
...
var verificate = function (data, isMighty, callback) { /*верификация сесии с проверками прав*/
	var reply;
	Userschema.findOne({_id: data._id}, 'session', function (err, person) {
		console.log(person);
			if (person !== null) {
			try {
				if (person.session == data.session  && person.session != 0) {
					console.log('verificate ok');
					Userschema.findOne({_id: data._id}, isMighty, function(err, person2){
						if (person2 !== null) {
...
var SECRET = "6LeZaREUAAAAAEAsbnMfCnljiWuhcv1M6m-BTYYa";
