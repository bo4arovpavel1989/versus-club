	...
	redisClient.sismember('votedLogin', login, function(err, rep){
		if (rep === 1) {
			redisClient.hgetall('voteCandidates', function(err, rep2){
				if (rep2 !== undefined) {
					var vote1 = Number(rep2.candidate001);
					var vote2 = Number(rep2.candidate002);
					var vote3 = Number(rep2.candidate003);
					var sumVote = vote1 + vote2 + vote3;
					if (sumVote !== 0) {
						var percV1 = (vote1 / sumVote);
						var percV2 = (vote2 / sumVote);
						var percV3 = (vote3 / sumVote);
						var data = {
							percV1: percV1 * 100,
							percV2: percV2 * 100,
							percV3: percV3 * 100,
